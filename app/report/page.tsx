...
export default function Report() {
  ...
  const model = useMemo(() => {
    ...
    // ---- savings by role (work that shifts to systems + senior oversight) ----
    const roles = ctx.staff
      .map((r) => {
        const ab = roleAbsorption(r.id, "base", ctx);
        return { label: r.label, today: ab.today, freed: ab.today - ab.residual, aid: ab.aidFrac };
      })
      .filter((r) => r.today > 0.01);
    roles.sort((x, y) => y.freed - x.freed);
  ...
  return (
    <div className="report mx-auto max-w-3xl bg-paper px-6 py-8 text-ink-900">
      ...
      <section className="report-section">
        ...
        <p className="mb-3 text-[12px] leading-snug text-ink-500">Read as <b>where the work goes</b>, not a cut list. The share of each role absorbed by systems is what we would <b>not</b> hire back — the residual is what we would.</p>
        <div className="grid grid-cols-2 gap-x-6 gap-y-1.5">
          {model.roles.slice(0, 12).map((r) => (
            <div key={r.label} className="report-avoid flex items-center gap-2">
              <div className="w-40 shrink-0 truncate text-[11px] font-medium text-ink-700">{r.label}</div>
              ...
              <span className="tnum w-9 shrink-0 text-right text-[10px] font-semibold text-accent-dark">{Math.round(r.aid * 100)}%</span>
            </div>
          ))}
        </div>
        ...
        <div className="mt-7">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { name: "Status Quo (Y3)", o: model.sq, tone: "neg" as const },
              { name: "Middle Path (Y3)", o: model.mid, tone: "mid" as const },
              { name: "Agency Zero (Y3)", o: model.zero, tone: "pos" as const },
            ].map((c) => (
              <div key={c.name} className={clsx("report-avoid rounded-xl border-2 bg-surface p-4", c.tone === "pos" ? "border-positive/40" : c.tone === "neg" ? "border-negative/50" : "border-rule")}>
                <div className="text-[11px] font-semibold text-ink-500">{c.name}</div>
                <div className={clsx("tnum mt-1 text-2xl font-extrabold", c.tone === "pos" ? "text-positive" : c.tone === "neg" ? "text-negative" : "text-ink-900")}>{fmtMoneyShort(c.o.profit)}</div>
          ...
          <p className="mt-3 text-[11px] text-ink-500">Even the cautious read clears today&apos;s ~{fmtMoneyShort(netProfitToday)}. The preferred use of the freed capacity is <b>growth</b> — same people, more revenue — not cuts.</p>
        </div>
      </section>
